...
class Employee {
                ...
                return this.getEmployeeList()
                    .then((employeeList) => {
                        return inquirer
                                    ...
                                    .then(detail => {
                                        return role.getRoleID(detail.roleName)
                                    ...
                                    .then(updatedDetail => {
                                        ...
                                        return db.promise().query(sql, params)
                                                .then(() => {
                                                    console.log(`Updated ${updatedDetail.name}'s role to ${updatedDetail.roleName}!`);
                                                })
                                                .catch((err) => console.log(err));
                                    })
                    })
                ...
                .then (managerList => {
                    return inquirer
                                ...
                                .then(updatedDetail => {
                                    ...
                                    return db.promise().query(sql, params)
                                            .then(() => {
                                                if (updatedDetail.name == "None") {
                                                    updatedDetail.name = "no one";
                                                }
    ...
    getBudget(){
        ...
        return department.getDepartmentList()
            .then(departmentList => {
                return inquirer
                    // Get the department user want to view the budget of
                    .prompt({
                        type: "list",
                    ...
                    .then(answer => {
                        return department.getDepartmentID(answer.departmentTarget)
                            .then(departmentID => {
                                var detail = {
                    ...
                    // Get and Calculate the total budget and Display
                    .then(departmentDetail => {
                        const sqlSalary = 'role.salary as "Salary"';

                        const sql = `${sqlSalary}`;

                        const joinRole = 'LEFT JOIN role ON employee.role_id = role.id';
                        const joinDepartment = 'LEFT JOIN department ON role.department_id = department.id';

                        const joinStatement = `${joinRole} ${joinDepartment}`;

                        const filtersql = `department.id = ${departmentDetail.id}`

                        return db.promise().query(`SELECT ${sql} FROM employee ${joinStatement} where ${filtersql}`)
                            .then(([rows,fields]) => {
                                var totalBudget = 0;
                                for (var i = 0; i < rows.length; i++) {
                                    totalBudget += parseInt(rows[i].Salary);
                                }
                                console.log(`Total Utilized Budget of ${departmentDetail.name} Department is $${totalBudget}`);
                            })
                            .catch((err) => {console.log(err)});
                                    })
                            })
    ...
    // Update the role of a group of employees
    updateGroupRole(oldRole, newRole) {
        const sql = `UPDATE employee SET role_id = ? WHERE role_id = ?`;
        const params = [newRole.id, oldRole.id];
        return db.promise().query(sql, params)
            .then(() => {
                console.log(`Updated employees from ${oldRole.name} to ${newRole.name}!`);
            })
            .catch((err) => console.log(err));
    }
